...
async function fetchSpotifyPlaylist(playlistId: string, token: string): Promise<PlaylistResult> {
...
async function queryPartnerPlaylist(token: string, uri: string, offset: number): Promise<any> {
...
async function fetchPartnerPlaylist(playlistId: string): Promise<PlaylistResult> {
...
export async function GET(event: RequestEvent) {
	const input = (event.url.searchParams.get('url') ?? '').trim();
	if (!input) {
		return json({ error: 'Nothing to load.' }, { status: 400 });
	}

	if (input.includes('\n')) {
		const tracks = parseManualList(input);
		if (tracks.length === 0) {
			return json({ error: 'No tracks found.' }, { status: 400 });
		}
		return json({
			name: 'Pasted list',
			tracks
	...
	if (userToken) {
		try {
			...
			if (result.tracks.length) {
				if (refreshed) {
					event.cookies.set(SESSION_COOKIE, JSON.stringify(refreshed), {
						httpOnly: true,
						path: '/',
